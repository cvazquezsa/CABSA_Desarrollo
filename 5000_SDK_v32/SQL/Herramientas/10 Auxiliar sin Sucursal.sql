        ...
        END ELSE BEGIN -- TASK1964
          EXEC dbo.sp_executesql N' 
            ...
            IF @@ROWCOUNT = 0
              INSERT SaldoU (/*Sucursal, */Empresa, Rama, Moneda, Grupo, Cuenta, SubCuenta, Saldo, SaldoU, PorConciliar, PorConciliarU, UltimoCambio, SubGrupo) -- BUG8803
                     VALUES (/*@Sucursal, */@Empresa, @Rama, @Moneda, @Grupo, @Cuenta, @SubCuenta, (@Cargo-@Abono), (@CargoU-@AbonoU), @ImporteConciliar, @CantidadConciliar, @Fecha, @SubGrupo)', -- BUG8803
            N'@Empresa varchar(5), @Rama varchar(50), @Moneda varchar(50), @Grupo varchar(50), @SubGrupo varchar(50), @Cuenta varchar(50),  @SubCuenta varchar(50),
			  @Cargo money, @Abono money, @CargoU float, @AbonoU float, @ImporteConciliar money, @CantidadConciliar money, @Fecha datetime',
		    @Empresa, @Rama, @Moneda, @Grupo, @SubGrupo, @Cuenta, @SubCuenta, @Cargo, @Abono, @CargoU, @AbonoU, @ImporteConciliar, @CantidadConciliar, @Fecha
        END -- TASK1964
      END
		...
		EXEC dbo.sp_executesql N'
          ...
          IF @@ROWCOUNT = 0
            INSERT AcumU (/*Sucursal, */Empresa,  Rama,  Moneda,  Cuenta,  SubCuenta,  Grupo,  Ejercicio,  Periodo,  Cargos,  Abonos, CargosU,  AbonosU, UltimoCambio, SubGrupo)
                  VALUES (/*@Sucursal, */@Empresa, @Rama, @Moneda, @Cuenta, @SubCuenta, @Grupo, @Ejercicio, @Periodo, @Cargo,  @Abono, @CargoU,  @AbonoU, @Fecha, @SubGrupo)',  -- BUG8803
          N'@Empresa varchar(5), @Rama varchar(5), @Moneda varchar(20), @Cuenta varchar(20), @SubCuenta varchar(50), @Grupo varchar(10),  @Ejercicio int, 
		   @Periodo int, @Cargo money, @Abono money, @CargoU float, @AbonoU float, @Fecha datetime, @SubGrupo varchar(20)',
		  @Empresa, @Rama, @Moneda, @Cuenta, @SubCuenta, @Grupo, @Ejercicio, @Periodo, @Cargo, @Abono, @CargoU, @AbonoU, @Fecha, @SubGrupo
...
AS BEGIN
  ...
  BEGIN TRANSACTION
	...
	END
    SELECT /*Sucursal, */Empresa, m.Rama, Moneda, Grupo, m.SubGrupo, Cuenta, SubCuenta, "Cargo" = CONVERT(money, SUM(Cargo)), "Abono" = CONVERT(money, SUM(Abono)), "CargoU" = CONVERT(float, SUM(CargoU)), "AbonoU" = CONVERT(float, SUM(AbonoU))
      INTO #sAuxiliarU
    ...
    DELETE AuxiliarUWMS 
     FROM AuxiliarUWMS m, DepurarMov t, Rama r
    WHERE t.Modulo = m.Modulo AND t.ID = m.ModuloID AND r.Rama = m.Rama AND (r.Conciliar = 0 OR m.Conciliado = 1)
    ...
    BEGIN
      ...
      SELECT /*Sucursal, */Empresa, m.Rama, 'Historico', '', m.Moneda, mon.TipoCambio, Grupo, SubGrupo, Cuenta, SubCuenta, @AnoAnterior, @MesAnterior, @FechaAnterior, Cargo, Abono, CargoU, AbonoU, 1, r.Conciliar
      ...
      SELECT /*Sucursal, */Empresa, m.Rama, 'Historico', '', m.Moneda, mon.TipoCambio, Grupo, m.SubGrupo, Cuenta, SubCuenta, @AnoAnterior, @MesAnterior, @FechaAnterior, Cargo, Abono, CargoU, AbonoU, 1, r.Conciliar
        FROM #sAuxiliarU m, Mon, Rama r
       WHERE Mon.Moneda = m.Moneda AND r.Rama = m.Rama  AND ISNULL(m.SubGrupo, '') <> ''

      ALTER TABLE AuxiliarUGral  ENABLE TRIGGER ALL
      ALTER TABLE AuxiliarUWMS  ENABLE TRIGGER ALL
    END ELSE BEGIN -- TASK1964
      EXEC dbo.sp_executesql N'
