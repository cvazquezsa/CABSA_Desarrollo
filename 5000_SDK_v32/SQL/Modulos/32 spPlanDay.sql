...
/*** PlanCentroDayRuta ***/
if exists (select * from sysobjects where id = object_id('dbo.PlanCentroDayRuta') and sysstat & 0xf = 2) drop view dbo.PlanCentroDayRuta
GO
CREATE VIEW PlanCentroDayRuta
--//WITH ENCRYPTION
AS 
SELECT Empresa, Ruta, "Periodo" = CONVERT(int, CASE WHEN FechaEntrega < GETDATE() THEN -1 ELSE ISNULL(DATEDIFF(Day, GETDATE(), FechaEntrega), 0) END), Estado, "Cantidad" = SUM(Cantidad)
  FROM PlanArtOP
 WHERE Accion = 'Producir' AND LiberacionID IS NULL
 GROUP BY Empresa, Ruta, CONVERT(int, CASE WHEN FechaEntrega < GETDATE() THEN -1 ELSE ISNULL(DATEDIFF(Day, GETDATE(), FechaEntrega), 0) END), Estado
UNION
  SELECT e.Empresa, d.Ruta, "Periodo" = CONVERT(int, CASE WHEN e.FechaEntrega < GETDATE() THEN -1 ELSE ISNULL(DATEDIFF(Day, GETDATE(), e.FechaEntrega), 0) END),
   ...
   GROUP BY e.Empresa, d.Ruta, CONVERT(int, CASE WHEN e.FechaEntrega < GETDATE() THEN -1 ELSE ISNULL(DATEDIFF(Day, GETDATE(), e.FechaEntrega), 0) END), e.Estatus
  HAVING SUM(CASE WHEN e.Estatus = 'PENDIENTE' THEN d.CantidadPendiente ELSE d.Cantidad END*d.Factor) > 0
GO


/*** PlanCentroDayCarga ***/
if exists (select * from sysobjects where id = object_id('dbo.PlanCentroDayCarga') and sysstat & 0xf = 2) drop view dbo.PlanCentroDayCarga
GO
CREATE VIEW PlanCentroDayCarga
--//WITH ENCRYPTION
AS 
SELECT p.Empresa, r.Centro, p.Periodo, p.Estado, "Carga" = ISNULL(SUM(r.TiempoFijo), 0.0) + ISNULL(SUM(r.TiempoVariable*p.Cantidad), 0.0)
  FROM PlanCentroDayRuta p, ProdRutaD r
 WHERE p.Ruta = r.Ruta
 GROUP BY p.Empresa, r.Centro, p.Periodo, p.Estado
GO

/*** PlanCentroDayCargaTotal ***/
if exists (select * from sysobjects where id = object_id('dbo.PlanCentroDayCargaTotal') and sysstat & 0xf = 2) drop view dbo.PlanCentroDayCargaTotal
GO
CREATE VIEW PlanCentroDayCargaTotal
--//WITH ENCRYPTION
AS 
SELECT Empresa, Centro, Periodo, "CargaTotal" = SUM(Carga)
  FROM PlanCentroDayCarga
...
/*** PlanCentroDayCapacidadCalc ***/
if exists (select * from sysobjects where id = object_id('dbo.PlanCentroDayCapacidadCalc') and sysstat & 0xf = 2) drop view dbo.PlanCentroDayCapacidadCalc
GO
CREATE VIEW PlanCentroDayCapacidadCalc
--//WITH ENCRYPTION
AS 
...
/*** PlanCentroDayCapacidadCalc10**/
if exists (select * from sysobjects where id = object_id('dbo.PlanCentroDayCapacidadCalc10') and sysstat & 0xf = 2) drop view dbo.PlanCentroDayCapacidadCalc10
GO
CREATE VIEW PlanCentroDayCapacidadCalc10
--//WITH ENCRYPTION
AS 
SELECT Centro, Capacidad, Periodo
  FROM PlanCentroDayCapacidadCalc, Plan10
GO


/*** PlanCentroDayCapacidadDif **/
if exists (select * from sysobjects where id = object_id('dbo.PlanCentroDayCapacidadDif') and sysstat & 0xf = 2) drop view dbo.PlanCentroDayCapacidadDif
GO
CREATE VIEW PlanCentroDayCapacidadDif
--//WITH ENCRYPTION
AS 
SELECT ct.Empresa, cap.Centro, cap.Periodo, "Diferencia" = ISNULL(SUM(cap.Capacidad), 0.0)-ISNULL(SUM(ct.CargaTotal), 0.0)
  FROM PlanCentroDayCapacidadCalc10 cap
  LEFT OUTER JOIN PlanCentroDayCargaTotal ct ON cap.Centro = ct.Centro AND cap.Periodo = ct.Periodo
 GROUP BY ct.Empresa, cap.Centro, cap.Periodo
GO

-- exec spPlanCentroDay 'DEMO'
/**************** spPlanCentroDay ****************/
if exists (select * from sysobjects where id = object_id('dbo.spPlanCentroDay') and type = 'P') drop procedure dbo.spPlanCentroDay
GO
CREATE PROCEDURE spPlanCentroDay
			@Empresa 	char(5),
			@Centro		char(10) = NULL
--//WITH ENCRYPTION
AS BEGIN 
  ...
  CREATE TABLE #PlanCentroDay (Centro varchar(10) COLLATE Database_Default NULL, Descripcion varchar(100) COLLATE Database_Default NULL, Tipo varchar(20) COLLATE Database_Default NULL, Periodo int NULL, Cantidad float NULL, Nombre varchar(20) COLLATE Database_Default NULL, Renglon int NULL)
  INSERT #PlanCentroDay (Centro, Descripcion, Tipo, Periodo, Cantidad, Nombre, Renglon)

  SELECT c.Centro, c.Descripcion, c.Tipo, p.Periodo, "Cantidad" = p.Carga, "Nombre" = p.Estado, Renglon = 1 
    FROM Centro c, PlanCentroDayCarga p
  ...
  SELECT c.Centro, c.Descripcion, c.Tipo, p.Periodo, "Cantidad" = ABS(p.Diferencia), "Nombre" = CASE WHEN p.Diferencia < 0.0 THEN 'Capacidad Faltante' ELSE 'Capacidad Excedente' END, Renglon = CASE WHEN p.Diferencia < 0.0 THEN 8 ELSE 7 END
    FROM Centro c, PlanCentroDayCapacidadDif p
   WHERE c.Centro = p.Centro AND p.Empresa = @Empresa AND p.Centro = CASE @Centro WHEN NULL THEN p.Centro ELSE @Centro END

  SELECT * FROM #PlanCentroDay
  ORDER BY Centro, Descripcion, Tipo, Renglon, Nombre
END
GO

/**************** spPlanArtDay ****************/
if exists (select * from sysobjects where id = object_id('dbo.spPlanArtDay') and type = 'P') drop procedure dbo.spPlanArtDay
GO
CREATE PROCEDURE spPlanArtDay
			@Empresa		char(5),
			@Periodos		int,
    			@Articulo		char(20),
			@Hoy			datetime,
...
AS BEGIN   
  ...
  IF @Referencia IS NOT NULL AND @ReferenciaModulo = 'PROY' --ARCC
  BEGIN
    INSERT PlanArt (Empresa, Almacen, Articulo, SubCuenta, Acronimo, Cantidad, Periodo)
...
/**************** spPlanDemandaDay ****************/
if exists (select * from sysobjects where id = object_id('dbo.spPlanDemandaDay') and type = 'P') drop procedure dbo.spPlanDemandaDay
GO
CREATE PROCEDURE spPlanDemandaDay
			@Estacion		int,
			@Empresa		char(5),
    			@Articulo		char(20),
    			@Almacen		char(10),
...
/*
EXEC spPlanDemandaDay 501, 'DEMO', 'A1', NULL, '3/6/3'
