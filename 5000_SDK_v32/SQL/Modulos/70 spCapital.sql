...
/**************** spCapitalVerificar ****************/
if exists (select * from sysobjects where id = object_id('dbo.spCapitalVerificar') and type = 'P') drop procedure dbo.spCapitalVerificar
GO
CREATE PROCEDURE spCapitalVerificar
    		    @ID               		int,
		    @Accion			char(20),
    		    @Empresa          		char(5),
		    @Usuario			char(10),
...
/**************** spCapitalAfectar ****************/
if exists (select * from sysobjects where id = object_id('dbo.spCapitalAfectar') and type = 'P') drop procedure dbo.spCapitalAfectar
GO             
CREATE PROCEDURE spCapitalAfectar
    		   @ID                		int,
...
AS BEGIN
  ...
  BEGIN
    ...
    IF @Ok IS NULL
    BEGIN
      INSERT CapitalD (Sucursal, ID,  Renglon, Socio, Titulo, Cantidad, Precio, Costo) 
      SELECT @Sucursal, @IDGenerar, Renglon, Socio, Titulo, Cantidad, Precio, Costo
        FROM CapitalD
       WHERE ID = @ID
  ...
  IF @Conexion = 0 
    ...
    BEGIN
      -- Generar Cxp a Nivel Socio
      IF @MovTipo IN ('CAP.AC', 'CAP.DC', 'CAP.CAP', 'CAP.DIV')
      BEGIN
        DECLARE crCapitalCxp CURSOR LOCAL FOR
         SELECT d.Socio, s.Cliente, s.Acreedor, SUM(Precio*Cantidad), SUM(Costo*Cantidad) 
           FROM CapitalD d, Socio s
          WHERE d.ID = @ID AND NULLIF(RTRIM(d.Socio), '') IS NOT NULL AND d.Socio = s.Socio
          GROUP BY d.Socio, s.Cliente, s.Acreedor
          ORDER BY d.Socio, s.Cliente, s.Acreedor

        OPEN crCapitalCxp
        FETCH NEXT FROM crCapitalCxp INTO @Socio, @Cliente, @Acreedor, @Precio, @Costo
        ...
        BEGIN
          ...
          BEGIN
            ...
            EXEC spGenerarCx @Sucursal, @SucursalOrigen, @SucursalDestino, @Accion, NULL, @Empresa, @Modulo, @ID, @Mov, @MovID, @MovTipo, @MovMoneda, @MovTipoCambio, 
                             @FechaEmision, NULL, @Proyecto, @Usuario, @Autorizacion, @Referencia, @DocFuente, @Observaciones,
  	    	             @FechaRegistro, @Ejercicio, @Periodo,
		             '(Fecha)', @FechaEmision, @Contacto, NULL, @Agente, NULL, NULL, NULL,
...
/**************** spCapital ****************/
if exists (select * from sysobjects where id = object_id('dbo.spCapital') and type = 'P') drop procedure dbo.spCapital
GO
CREATE PROCEDURE spCapital
                   @ID                  	int,
    		   @Modulo	      		char(5),
		   @Accion			char(20),
		   @Base			char(20),
...
AS BEGIN
  ...
  SELECT @Sucursal = Sucursal, @SucursalDestino = SucursalDestino, @SucursalOrigen = SucursalOrigen, @Empresa = Empresa, @MovID = MovID, @Mov = Mov, @FechaEmision = FechaEmision, @Proyecto = NULLIF(RTRIM(Proyecto), ''),
         ...
         @Concepto = Concepto, @Referencia = Referencia, @Agente = Agente
    FROM Capital
   WHERE ID = @ID
  IF @@ERROR <> 0 SELECT @Ok = 1
  ...
  BEGIN
    ...
    BEGIN
      EXEC spCapitalVerificar @ID, @Accion, @Empresa, @Usuario, @Modulo, @Mov, @MovID, @MovTipo, @MovMoneda, @MovTipoCambio, @FechaEmision, @Estatus, @EstatusNuevo, 
    ...
    IF @Accion IN ('AFECTAR', 'GENERAR', 'CANCELAR','CONSECUTIVO','SINCRO') AND @Ok IS NULL
      EXEC spCapitalAfectar @ID, @Accion, @Empresa, @Modulo, @Mov, @MovID OUTPUT, @MovTipo, @MovMoneda, @MovTipoCambio, @FechaEmision, @FechaAfectacion, @FechaConclusion,
			     @Proyecto, @Usuario, @Autorizacion, @DocFuente, @Observaciones, @Concepto, @Referencia, @Agente,
