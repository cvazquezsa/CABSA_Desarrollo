...
--    EXEC spFormaExtraEnviarCorreo @Empresa, @FormaTipo, @ID, 'CAPTURA', @Ok OUTPUT, @OkRef OUTPUT

/**************** spFormaExtraEnviarCorreo ****************/
if exists (select * from sysobjects where id = object_id('dbo.spFormaExtraEnviarCorreo') and type = 'P') drop procedure dbo.spFormaExtraEnviarCorreo
GO
CREATE PROCEDURE spFormaExtraEnviarCorreo
		    @Empresa		varchar(5),
		    @FormaTipo		varchar(50),
    		    @ID         	int,
		    @Usar		varchar(20),	-- CAPTURA, INFO
    		    @eMail		varchar(50),
...
AS BEGIN
  DECLARE
    ...
    @Adjunto	varchar(8000),
    @Archivo	varchar(255),
    @Asunto	varchar(100)

  SELECT @Perfil = NULLIF(RTRIM(DBMailPerfil), '') FROM EmpresaGral WHERE Empresa = @Empresa
  IF @Perfil IS NULL RETURN 

  IF UPPER(@Usar) = 'CAPTURA'
  BEGIN
    SELECT @Mensaje = NULLIF(RTRIM(CONVERT(varchar(8000), MensajeHTML)), ''), 
           @Adjunto = NULLIF(RTRIM(CONVERT(varchar(8000), CapturaHTML)), ''), 
           @Asunto  = Asunto,
           @Archivo = ArchivoAdjunto
      FROM FormaTipo
     WHERE FormaTipo = @FormaTipo

    EXEC spFormaParcearHTML @ID, @Ok, @OkRef, @Mensaje OUTPUT    
    EXEC spFormaParcearHTML @ID, @Ok, @OkRef, @Adjunto OUTPUT
    EXEC spTextoEnArchivo @Adjunto, @Archivo
    EXEC spEnviarDBMail @Perfil, @eMail, @Asunto = @Asunto, @Mensaje = @Mensaje--, @Adjuntos = @Archivo
  END ELSE 
  BEGIN
    SELECT @Mensaje = NULLIF(RTRIM(CONVERT(varchar(8000), InfoHTML)), ''),
           @Asunto  = Asunto
      FROM FormaTipo
     WHERE FormaTipo = @FormaTipo
    EXEC spFormaParcearHTML @ID, @Ok, @OkRef, @Mensaje OUTPUT
    EXEC spEnviarDBMail @Perfil, @eMail, @Asunto = @Asunto, @Mensaje = @Mensaje
  END
  RETURN
END
GO

/**************** spFormaExtraVerificar ****************/
if exists (select * from sysobjects where id = object_id('dbo.spFormaExtraVerificar') and type = 'P') drop procedure dbo.spFormaExtraVerificar
GO
CREATE PROCEDURE spFormaExtraVerificar
...
AS BEGIN
  ...
  END ELSE
  BEGIN
    IF @FormaTipo   IS NULL SELECT @Ok = 53010 ELSE
    IF @Aplica      IS NULL SELECT @Ok = 53020 ELSE
    IF @AplicaClave IS NULL SELECT @Ok = 53030 ELSE
    IF @MovTipo = 'FRM.E' AND @eMail IS NULL SELECT @Ok = 53050
  END
...
/**************** spFormaExtraAfectar ****************/
if exists (select * from sysobjects where id = object_id('dbo.spFormaExtraAfectar') and type = 'P') drop procedure dbo.spFormaExtraAfectar
GO             
CREATE PROCEDURE spFormaExtraAfectar
    		   @ID                		int,
...
/**************** spFormaExtra ****************/
if exists (select * from sysobjects where id = object_id('dbo.spFormaExtra') and type = 'P') drop procedure dbo.spFormaExtra
GO
CREATE PROCEDURE spFormaExtra
                   @ID                  	int,
    		   @Modulo	      		char(5),
		   @Accion			char(20),
		   @Base			char(20),
...
AS BEGIN
  ...
  SELECT @Sucursal = Sucursal, @SucursalDestino = SucursalDestino, @SucursalOrigen = SucursalOrigen, @Empresa = Empresa, @MovID = MovID, @Mov = Mov, @FechaEmision = FechaEmision, @Proyecto = NULLIF(RTRIM(Proyecto), ''), @UEN = UEN,
         ...
         @FormaTipo = NULLIF(RTRIM(FormaTipo), ''), @Aplica = NULLIF(RTRIM(Aplica), ''), @AplicaClave = NULLIF(RTRIM(AplicaClave), ''),
         @eMail = NULLIF(RTRIM(eMail), '')
    FROM FormaExtra
   WHERE ID = @ID
  IF @@ERROR <> 0 SELECT @Ok = 1
  ...
  IF (@Accion <> 'CANCELAR' AND @Estatus IN ('SINAFECTAR', 'CONFIRMAR', 'BORRADOR', 'EMAIL', 'PENDIENTE')) OR 
  ...
  BEGIN
    ...
    ELSE BEGIN
      IF @MovTipo = 'FRM.E' AND @Estatus = 'SINAFECTAR' 
        SELECT @EstatusNuevo = 'EMAIL' 
      ELSE
    ...
    BEGIN
      EXEC spFormaExtraVerificar @ID, @Accion, @Empresa, @Usuario, @Modulo, @Mov, @MovID, @MovTipo, @MovMoneda, @MovTipoCambio, @FechaEmision, @Estatus, @EstatusNuevo, 
                            @FormaTipo, @Aplica, @AplicaClave, @eMail, 
    ...
    IF @Accion IN ('AFECTAR', 'GENERAR', 'CANCELAR','CONSECUTIVO','SINCRO') AND @Ok IS NULL
      EXEC spFormaExtraAfectar @ID, @Accion, @Base, @Empresa, @Modulo, @Mov, @MovID OUTPUT, @MovTipo, @MovMoneda, @MovTipoCambio, @FechaEmision, @FechaAfectacion, @FechaConclusion,
  ...
  END ELSE 
  BEGIN
    IF @Estatus IN ('SINAFECTAR', 'CONFIRMAR', 'BORRADOR', 'EMAIL') AND @Accion = 'CANCELAR' EXEC spMovCancelarSinAfectar @Modulo, @ID, @Ok OUTPUT ELSE
