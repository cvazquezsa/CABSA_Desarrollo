/**************** spPresupVerificar ****************/
if exists (select * from sysobjects where id = object_id('dbo.spPresupVerificar') and type = 'P') drop procedure dbo.spPresupVerificar
GO
CREATE PROCEDURE spPresupVerificar
    		    @ID               		int,
		    @Accion			char(20),
    		    @Empresa          		char(5),
		    @Usuario			char(10),
...
/**************** spPresupAfectar ****************/
if exists (select * from sysobjects where id = object_id('dbo.spPresupAfectar') and type = 'P') drop procedure dbo.spPresupAfectar
GO             
CREATE PROCEDURE spPresupAfectar
    		   @ID                		int,
...
/**************** spPresup ****************/
if exists (select * from sysobjects where id = object_id('dbo.spPresup') and type = 'P') drop procedure dbo.spPresup
GO
CREATE PROCEDURE spPresup
                   @ID                  	int,
    		   @Modulo	      		char(5),
		   @Accion			char(20),
		   @Base			char(20),
...
AS BEGIN
  ...
  BEGIN
    ...
    BEGIN
      EXEC spPresupVerificar @ID, @Accion, @Empresa, @Usuario, @Modulo, @Mov, @MovID, @MovTipo, @MovMoneda, @MovTipoCambio, @FechaEmision, @Estatus, @EstatusNuevo, 
    ...
    IF @Accion IN ('AFECTAR', 'GENERAR', 'CANCELAR','CONSECUTIVO','SINCRO') AND @Ok IS NULL
      EXEC spPresupAfectar @ID, @Accion, @Empresa, @Modulo, @Mov, @MovID OUTPUT, @MovTipo, @MovMoneda, @MovTipoCambio, @FechaEmision, @FechaAfectacion, @FechaConclusion,
