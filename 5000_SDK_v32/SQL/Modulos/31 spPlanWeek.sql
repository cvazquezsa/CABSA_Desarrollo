...
/*** PlanCentroWeekRuta ***/
if exists (select * from sysobjects where id = object_id('dbo.PlanCentroWeekRuta') and sysstat & 0xf = 2) drop view dbo.PlanCentroWeekRuta
GO
CREATE VIEW PlanCentroWeekRuta
--//WITH ENCRYPTION
AS 
SELECT Empresa, Ruta, "Periodo" = CONVERT(int, CASE WHEN FechaEntrega < GETDATE() THEN -1 ELSE ISNULL(DATEDIFF(week, GETDATE(), FechaEntrega), 0) END), Estado, "Cantidad" = SUM(Cantidad)
  FROM PlanArtOP
 WHERE Accion = 'Producir' AND LiberacionID IS NULL
 GROUP BY Empresa, Ruta, CONVERT(int, CASE WHEN FechaEntrega < GETDATE() THEN -1 ELSE ISNULL(DATEDIFF(week, GETDATE(), FechaEntrega), 0) END), Estado
UNION
  SELECT e.Empresa, d.Ruta, "Periodo" = CONVERT(int, CASE WHEN e.FechaEntrega < GETDATE() THEN -1 ELSE ISNULL(DATEDIFF(week, GETDATE(), e.FechaEntrega), 0) END),
   ...
   GROUP BY e.Empresa, d.Ruta, CONVERT(int, CASE WHEN e.FechaEntrega < GETDATE() THEN -1 ELSE ISNULL(DATEDIFF(week, GETDATE(), e.FechaEntrega), 0) END), e.Estatus
  HAVING SUM(CASE WHEN e.Estatus = 'PENDIENTE' THEN d.CantidadPendiente ELSE d.Cantidad END*d.Factor) > 0
GO


/*** PlanCentroWeekCarga ***/
if exists (select * from sysobjects where id = object_id('dbo.PlanCentroWeekCarga') and sysstat & 0xf = 2) drop view dbo.PlanCentroWeekCarga
GO
CREATE VIEW PlanCentroWeekCarga
--//WITH ENCRYPTION
AS 
SELECT p.Empresa, r.Centro, p.Periodo, p.Estado, "Carga" = ISNULL(SUM(r.TiempoFijo), 0.0) + ISNULL(SUM(r.TiempoVariable*p.Cantidad), 0.0)
  FROM PlanCentroWeekRuta p, ProdRutaD r
 WHERE p.Ruta = r.Ruta
 GROUP BY p.Empresa, r.Centro, p.Periodo, p.Estado
GO

/*** PlanCentroWeekCargaTotal ***/
if exists (select * from sysobjects where id = object_id('dbo.PlanCentroWeekCargaTotal') and sysstat & 0xf = 2) drop view dbo.PlanCentroWeekCargaTotal
GO
CREATE VIEW PlanCentroWeekCargaTotal
--//WITH ENCRYPTION
AS 
SELECT Empresa, Centro, Periodo, "CargaTotal" = SUM(Carga)
  FROM PlanCentroWeekCarga
...
/*** PlanCentroWeekCapacidadCalc ***/
if exists (select * from sysobjects where id = object_id('dbo.PlanCentroWeekCapacidadCalc') and sysstat & 0xf = 2) drop view dbo.PlanCentroWeekCapacidadCalc
GO
CREATE VIEW PlanCentroWeekCapacidadCalc
--//WITH ENCRYPTION
AS 
...
/*** PlanCentroWeekCapacidadCalc10**/
if exists (select * from sysobjects where id = object_id('dbo.PlanCentroWeekCapacidadCalc10') and sysstat & 0xf = 2) drop view dbo.PlanCentroWeekCapacidadCalc10
GO
CREATE VIEW PlanCentroWeekCapacidadCalc10
--//WITH ENCRYPTION
AS 
SELECT Centro, Capacidad, Periodo
  FROM PlanCentroWeekCapacidadCalc, Plan10
GO


/*** PlanCentroWeekCapacidadDif **/
if exists (select * from sysobjects where id = object_id('dbo.PlanCentroWeekCapacidadDif') and sysstat & 0xf = 2) drop view dbo.PlanCentroWeekCapacidadDif
GO
CREATE VIEW PlanCentroWeekCapacidadDif
--//WITH ENCRYPTION
AS 
SELECT ct.Empresa, cap.Centro, cap.Periodo, "Diferencia" = ISNULL(SUM(cap.Capacidad), 0.0)-ISNULL(SUM(ct.CargaTotal), 0.0)
  FROM PlanCentroWeekCapacidadCalc10 cap
  LEFT OUTER JOIN PlanCentroWeekCargaTotal ct ON cap.Centro = ct.Centro AND cap.Periodo = ct.Periodo
 GROUP BY ct.Empresa, cap.Centro, cap.Periodo
GO

-- exec spPlanCentroWeek 'DEMO'
/**************** spPlanCentroWeek ****************/
if exists (select * from sysobjects where id = object_id('dbo.spPlanCentroWeek') and type = 'P') drop procedure dbo.spPlanCentroWeek
GO
CREATE PROCEDURE spPlanCentroWeek
			@Empresa 	char(5),
			@Centro		char(10) = NULL
--//WITH ENCRYPTION
AS BEGIN 
  ...
  CREATE TABLE #PlanCentroWeek (Centro varchar(10) COLLATE Database_Default NULL, Descripcion varchar(100) COLLATE Database_Default NULL, Tipo varchar(20) COLLATE Database_Default NULL, Periodo int NULL, Cantidad float NULL, Nombre varchar(20) COLLATE Database_Default NULL, Renglon int NULL)
  INSERT #PlanCentroWeek (Centro, Descripcion, Tipo, Periodo, Cantidad, Nombre, Renglon)

  SELECT c.Centro, c.Descripcion, c.Tipo, p.Periodo, "Cantidad" = p.Carga, "Nombre" = p.Estado, Renglon = 1 
    FROM Centro c, PlanCentroWeekCarga p
  ...
  SELECT c.Centro, c.Descripcion, c.Tipo, p.Periodo, "Cantidad" = ABS(p.Diferencia), "Nombre" = CASE WHEN p.Diferencia < 0.0 THEN 'Capacidad Faltante' ELSE 'Capacidad Excedente' END, Renglon = CASE WHEN p.Diferencia < 0.0 THEN 8 ELSE 7 END
    FROM Centro c, PlanCentroWeekCapacidadDif p
   WHERE c.Centro = p.Centro AND p.Empresa = @Empresa AND p.Centro = CASE @Centro WHEN NULL THEN p.Centro ELSE @Centro END

  SELECT * FROM #PlanCentroWeek
  ORDER BY Centro, Descripcion, Tipo, Renglon, Nombre
END
GO

/**************** spPlanArtWeek ****************/
if exists (select * from sysobjects where id = object_id('dbo.spPlanArtWeek') and type = 'P') drop procedure dbo.spPlanArtWeek
GO
CREATE PROCEDURE spPlanArtWeek
			@Empresa		char(5),
			@Periodos		int,
    			@Articulo		char(20),
			@Hoy			datetime,
...
/**************** spPlanDemandaWeek ****************/
if exists (select * from sysobjects where id = object_id('dbo.spPlanDemandaWeek') and type = 'P') drop procedure dbo.spPlanDemandaWeek
GO
CREATE PROCEDURE spPlanDemandaWeek
			@Estacion		int,
			@Empresa		char(5),
    			@Articulo		char(20),
    			@Almacen		char(10),
...
/*
EXEC spPlanDemandaWeek 501, 'DEMO', 'A1', NULL, '3/6/3'
