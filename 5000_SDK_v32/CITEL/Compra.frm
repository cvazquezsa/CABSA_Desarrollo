...
CondicionVisible=(Usuario.Costos o (MovTipo(<T>COMS<T>,Compra:Compra.Mov)=COMS.FL)) y (MovTipo(<T>COMS<T>,Compra:Compra.Mov) noen (COMS.OG, COMS.IG, COMS.DG)) y ((fnSubMovTipo(<T>COMS<T>, Compra:Compra.Mov) <> <T>COMS.EIMPO<T>))
...
Antes=S
AntesExpresiones=Asigna( Info.ID, Compra:Compra.ID )
[Acciones.Verificar]
Nombre=Verificar
Boton=41
Menu=&Archivo
...
ActivoCondicion=(Config.SeriesLotesMayoreo) y (MovTipoEn(<T>COMS<T>, Compra:Compra.Mov, (COMS.F,COMS.FL,COMS.EG,COMS.EI,COMS.IG,COMS.D,COMS.DG,COMS.CC,COMS.DC,COMS.B,COMS.CA,COMS.GX)) o (fnSubMovTipo(<T>COMS<T>, Compra:Compra.Mov) = <T>COMS.EIMPO<T>))
...
VisibleCondicion=(Config.SeriesLotesMayoreo) y (Compra:Compra.Estatus en (EstatusSinAfectar,EstatusBorrador)) y (MovTipoEn(<T>COMS<T>, Compra:Compra.Mov, (COMS.F, COMS.FL, COMS.EG, COMS.EI, COMS.CC, COMS.IG)) o (fnSubMovTipo(<T>COMS<T>, Compra:Compra.Mov) = <T>COMS.EIMPO<T>))
...
VisibleCondicion=(MovTipo(<T>COMS<T>, Compra:Compra.Mov) en (COMS.EG,COMS.EI,COMS.PR,COMS.OI, COMS.GX)) o (fnSubmovTipo(<T>COMS<T>, Compra:Compra.Mov) = <T>COMS.EIMPO<T>)
...
CondicionVisible=(MovTipoEn(<T>COMS<T>, Compra:Compra.Mov, COMS.EI)) o (fnSubMovTipo(<T>COMS<T>, Compra:Compra.Mov) = <T>COMS.EIMPO<T>)
...
ImporteTotal=Compra.MovIntelisisMES
Compra.MovIntelisisMES=(Fin)

[(Carpeta Abrir).Compra.MovIntelisisMES]
Carpeta=(Carpeta Abrir)
Clave=Compra.MovIntelisisMES
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
...
AplicaNombre=CompraD.MonedaD
CompraD.MonedaD=CompraD.TipoCambioD
CompraD.TipoCambioD=CompraD.Destino
...
[Ficha.Compra.CrossDocking]
Carpeta=Ficha
Clave=Compra.CrossDocking
Editar=S
LineaNueva=N
ValidaNombre=S
3D=S
...
Menu=&Edición
Antes=S
ClaveAccion=CompraActivoFijo
GuardarAntes=S
RefrescarDespues=S
ActivoCondicion=SQL( <T>SELECT clave FROM MovTipo WHERE Modulo = :tModulo AND Mov = :tMov<T>,<T>COMS<T>,Compra:Compra.Mov ) EN (<T>COMS.EI<T>,<T>COMS.EG<T>,<T>COMS.B<T>,<T>COMS.D<T>) y<BR>SQL( <T>SELECT COUNT(*) FROM CompraGastoDiverso WHERE ID = :nID<T>, Compra:Compra.ID) >= 1 y<BR>SQL( <T>SELECT COUNT(*) FROM SerieLoteMov WHERE Modulo = :tModulo AND ID = :nID<T>, <T>COMS<T>, Compra:Compra.ID) >= 1
AntesExpresiones=Asigna( Info.ID, Compra:Compra.ID )<BR>Asigna( Info.Modulo, <T>COMS<T> )<BR>Asigna( Info.Mov, Compra:Compra.Mov )<BR>Asigna( Info.Estatus, Compra:Compra.Estatus )
...
Compra.ReferenciaMES=Compra.MovIntelisisMES
Compra.MovIntelisisMES=Compra.PosicionWMS
Compra.PosicionWMS=Compra.CrossDocking
Compra.CrossDocking=Compra.Directo
...
ActivoCondicion=Compra:Compra.Estatus<><T>SINAFECTAR<T>
VisibleCondicion=SQL(<T>EXEC spRepMovPendientesSurtidoExiste :tModulo, :tMov<T>, <T>COMS<T>, Compra:Compra.Mov)=1
[Acciones.RepPendientesSurtido.Variables]
Nombre=Variables
Boton=0
TipoAccion=Expresion
Expresion=Asigna(Info.Modulo, <T>COMS<T>)<BR>Asigna(Info.ID, Compra:Compra.ID)
Activo=S
Visible=S
...
[Detalle.CompraD.MonedaD]
Carpeta=Detalle
Clave=CompraD.MonedaD
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=10
...
[Detalle.CompraD.TipoCambioD]
Carpeta=Detalle
Clave=CompraD.TipoCambioD
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
...
[Acciones.RepPantalla.asignaVar]
Nombre=asignaVar
Boton=0
TipoAccion=Expresion
Expresion=Asigna( Info.ID, CompraD:CompraD.ID )
Activo=S
Visible=S

[Acciones.RepPantalla.Reporte]
Nombre=Reporte
Boton=0
TipoAccion=Expresion
...
MovImpuesto=Tiempo
Tiempo=RepPendientesSurtido
RepPendientesSurtido=Politica
