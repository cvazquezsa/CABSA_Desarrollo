...
TituloAutoNombre=S
Menus=S
PosicionInicialAlturaCliente=957
Totalizadores=S
PosicionSec1=373
PosicionSec2=861
...
DespuesGuardar=S
Activo=S
AntesExpresiones=Asigna(Afectar.Modulo, <T>VTAS<T>)<BR>Asigna(Afectar.ID, Venta:Venta.ID)<BR>Asigna(Afectar.Mov, Venta:Venta.Mov)<BR>Asigna(Afectar.MovID, Venta:Venta.MovID)<BR>Asigna(Info.Cliente, Venta:Venta.Cliente)<BR>Asigna(Info.EnviarA, Venta:Venta.EnviarA)<BR>Asigna(Info.ID, Venta:Venta.ID)<BR>Asigna(Info.Cliente, Venta:Venta.Cliente)<BR>Asigna(Info.Fecha, Venta:Venta.FechaEmision)<BR>Asigna(Info.Moneda, Venta:Venta.Moneda)<BR>Asigna(Info.TipoCambio,Venta:Venta.TipoCambio)<BR>Asigna(Info.Importe, Redondea(Suma(VentaD:Importe)+Suma(VentaD:Impuesto)-Suma(VentaD:DescGlobal)+Suma(VentaD:ImporteSobrePrecio)-(Venta:Venta.AnticiposFacturados+Venta:Venta.Retencion), Config.VentaCobroRedondeoDecimales))<BR>Asigna(Info.Cerrado, Venta:Venta.Estatus<>EstatusSinAfectar)<BR>Asigna(Info.VentaCobroDevolucion, MovTipoEn(<T>VTAS<T>, Venta:Venta.Mov, (VTAS.VP, VTAS.SD, VTAS.D, VTAS.DF)))<BR>Asigna(Info.FormaPago, Vacio(Usuario.DefFormaPago, Config.FormaPagoOmision))
...
Expresion=Si<BR>    ConDatos(Venta:Venta.PedidoReferenciaID)<BR>Entonces<BR>    Asigna(Info.RID,Venta:Venta.PedidoReferenciaID)<BR>    Si<BR>        Forma(<T>CxcAnticiposPendientes2<T>)<BR>    Entonces<BR>        Si<BR>            Config.CxcAnticipoTipoServicio = Verdadero<BR>        Entonces<BR>            SQL(<T>EXEC spVtasAplicarAnticiposTipoServicio :nEstacion, :nID<T>,EstacionTrabajo, Venta:Venta.ID)<BR>            ActualizarForma<BR>        SiNo<BR>            Asigna(Temp.Reg, SQL(<T>SELECT SUM(AnticipoAplicar), SUM(AnticipoAplicar*Impuestos/(Importe+Impuestos)) FROM Cxc WHERE AnticipoAplicaModulo=:tModulo AND AnticipoAplicaID=:nID<T>, Info.Modulo, Info.ID))<BR>            Asigna(Venta:Venta.AnticiposFacturados, Temp.Reg[1])<BR>            Asigna(Venta:Venta.AnticiposImpuestos,  Temp.Reg[2])<BR>        Fin<BR>    Fin<BR>Sino<BR>    Si<BR>        Forma(<T>CxcAnticiposPendientes<T>)<BR>    Entonces<BR>        ProcesarSQL(<T>EXEC spAplicarAnticiposCFDI :nEstacion, :nID<T>,EstacionTrabajo, Venta:Venta.ID)<BR>        ActualizarForma<BR>    Fin<BR>Fin
...
[(Carpeta Totalizadores).Puntos Ganados]
Carpeta=(Carpeta Totalizadores)
Clave=Puntos Ganados
Editar=S
LineaNueva=N
ValidaNombre=S
3D=S
...
[Ficha.Venta.FolioCRM]
Carpeta=Ficha
Clave=Venta.FolioCRM
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
...
[Acciones.UbicacionEmbarque]
Nombre=UbicacionEmbarque
Boton=0
NombreDesplegar=Ubicacion Embarque
GuardarAntes=S
EnMenu=S
TipoAccion=Formas
ClaveAccion=VentaDEmbarques
Antes=S
DespuesGuardar=S
Visible=S
...
ActivoCondicion=MovTipoEn(<T>VTAS<T>, Venta:Venta.Mov,(<T>VTAS.EST<T>)) Venta:Venta.Estatus = <T>CONCLUIDO<T>
AntesExpresiones=Asigna(Info.Articulo, VentaD:VentaD.Articulo)<BR>Asigna(Info.ID, Venta:Venta.ID)<BR>Asigna(Info.RenglonID, VentaD:VentaD.RenglonID)<BR>Asigna(Info.Renglon, VentaD:VentaD.Renglon)
...
[Ficha.Venta.PedidoReferencia]
Carpeta=Ficha
Clave=Venta.PedidoReferencia
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
...
VentaD.LDIReferencia=VentaD.Puntos
VentaD.Puntos=(Fin)
...
[Ficha.Venta.Monedero]
Carpeta=Ficha
Clave=Venta.Monedero
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
...
[Acciones.MonederoCBSaldo]
Nombre=MonederoCBSaldo
Boton=0
NombreEnBoton=S
NombreDesplegar=Saldo Monedero
EnBarraHerramientas=S
EnMenu=S
TipoAccion=Expresion
Activo=S
Antes=S
Expresion=Si<BR>  ConDatos(Info.MonederoCBSerie)<BR>Entonces<BR>   ReportePantalla(<T>RepSaldoMonederoCB<T>)<BR>Sino<BR>  Informacion(<T>NO HA ASIGNADO NINGUN MONEDERO<T>)<BR>Fin
AntesExpresiones=Asigna(Info.MonederoCBSerie,Venta:Venta.Monedero)
VisibleCondicion=Info.MonederoCB
...
Venta.Directo=Venta.PedidoReferencia
Venta.PedidoReferencia=Venta.FolioCRM
Venta.FolioCRM=Venta.Monedero
Venta.Monedero=Venta.ConceptoFE
Venta.ConceptoFE=(Fin)

[Ficha.Venta.ConceptoFE]
Carpeta=Ficha
Clave=Venta.ConceptoFE
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
...
[Acciones.CFDRelacionadoVentaSaldo]
...
[Acciones.Prorrateo.Variables]
Nombre=Variables
Boton=0
TipoAccion=Expresion
Expresion=Asigna(Info.ID, Venta:Venta.ID)<BR>Asigna(Info.Importe, Venta:Venta.Importe)
Activo=S
Visible=S

[Acciones.Prorrateo.VtasProrrateo]
Nombre=VtasProrrateo
Boton=0
TipoAccion=Formas
ClaveAccion=VtasProrrateo
Activo=S
Visible=S
...
[Acciones.Prorrateo.ListaAccionesMultiples]
(Inicio)=Variables
...
ContParalelaMovDato=UbicacionEmbarque
UbicacionEmbarque=MonederoCBSaldo
MonederoCBSaldo=Prorrateo
Prorrateo=(Fin)
