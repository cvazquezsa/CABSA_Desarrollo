[Forma]
Clave=RepCxcPronostico
Nombre=Pronóstico de Cobranza
Icono=0
Modulos=(Todos)
...
ExpresionesAlMostrar=Asigna(Info.ClienteD, SQL(<T>SELECT MIN(Cliente) FROM Cte<T>))<BR>Asigna(Info.ClienteA, SQL(<T>SELECT MAX(Cliente) FROM Cte<T>))<BR>Asigna(Info.Desglosar, <T>Si<T>)<BR>Asigna(Info.Moneda, <T>(Todas)<T>)<BR>Asigna(Info.Ano, Año(Hoy))<BR>Asigna(Rep.Titulo, <T>Pronóstico de Cobranza<T>)
[(Variables)]
Estilo=Ficha
Clave=(Variables)
PermiteEditar=S
...
Info.Desglosar=Info.Ano
Info.Ano=Info.Moneda
