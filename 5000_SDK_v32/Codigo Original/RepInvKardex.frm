...
ExpresionesAlMostrar=Asigna(Info.ArticuloD, SQL(<T>SELECT MIN(Articulo) FROM Art<T>))<BR>Asigna(Info.ArticuloA, SQL(<T>SELECT MAX(Articulo) FROM Art<T>))<BR>Asigna(Info.FechaD, PrimerDiaMes)<BR>Asigna(Info.FechaA, UltimoDiaMes )<BR>Asigna(Info.Moneda, Config.ContMoneda)<BR>Asigna(Rep.Aux, <T>Concentrado<T>)<BR>Asigna(Rep.MovEspecifico, <T>(Todos)<T>)<BR>Asigna(Rep.InvAuxUMov, <T>(Todos)<T>)<BR>Asigna(Info.SubCuenta, <T>(Todas)<T>)<BR>Asigna(Info.SerieLote, <T>(Todos)<T>)<BR>Asigna(Info.ArtCat, <T>(Todos)<T>)<BR>Asigna(Info.ArtFam, <T>(Todos)<T>)<BR>Asigna(Info.ArtGrupo, <T>(Todos)<T>)<BR>Asigna(Info.Fabricante, <T>(Todos)<T>)<BR>Asigna(Info.ArtLinea, <T>(Todos)<T>)<BR>Asigna(Rep.InvAcumU, <T>Inventario<T>)<BR>Asigna(Rep.InvVal, <T>(Sin Valuar)<T>)<BR>Asigna(Info.Almacen, <T>(Todos)<T>)<BR>Asigna(Rep.Titulo, <T>Kardex - Inventarios<T>)
[(Variables)]
Estilo=Ficha
Clave=(Variables)
...
Expresion=ReporteImpresora(<T>REpInvKardex<T>)
[Acciones.Imprimir]
Nombre=Imprimir
Boton=4
...
Info.SubCuenta=Info.SerieLote
Info.SerieLote=Info.ArtCat
...
Rep.Titulo=Info.Reporte
Info.Reporte=(Fin)

[(Variables).Info.Reporte]
Carpeta=(Variables)
Clave=Info.Reporte
LineaNueva=S
ValidaNombre=S
...
(Inicio)=Variables Asignar
Variables Asignar=RepInvKardex
RepInvKardex=(Fin)
