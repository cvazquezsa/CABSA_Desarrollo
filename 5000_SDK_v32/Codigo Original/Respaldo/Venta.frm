...
VentanaColor=Plata
ExpresionesAlMostrar=Asigna(Info.Copiar, Falso)<BR>Asigna(Info.Positivos, Verdadero)<BR>Asigna(Info.Paquete, Nulo)<BR>Asigna(Info.RefrescandoPrecio, Falso)<BR>Asigna(Info.ABC, Nulo)<BR>Asigna(Info.MonederoCB,SQL(<T>SELECT ISNULL(VentaMonederoA,0) FROM EmpresaCfg2 WHERE Empresa = :tEmpresa<T>, Empresa))
