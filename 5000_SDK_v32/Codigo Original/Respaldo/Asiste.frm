...
ActivoCondicion=Asiste.Estatus = <T>CONCLUIDO<T>
AntesExpresiones=Asigna(Info.TareaID,0)<BR>Asigna(Info.ID,Asiste:Asiste.ID)
...
TipoAccion=Otros
ClaveAccion=Mapa
ListaParametros1=AsisteD:AsisteD.MapaLatitud<BR>AsisteD:AsisteD.MapaLongitud<BR> <BR> <BR>AsisteD:AsisteD.MapaUbicacion
ListaParametros=S
Activo=S
...
VisibleCondicion=(MovTipo(<T>ASIS<T>, Asiste:Asiste.Mov) en (ASIS.R, ASIS.PA, ASIS.A)) y (Asiste:Asiste.Estatus = EstatusSinAfectar)
[Acciones.Ubicacion2]
Nombre=Ubicacion2
Boton=0
NombreDesplegar=Consulta Ubicación
EnMenu=S
TipoAccion=Expresion
Activo=S
...
Expresion=Forma.Mapa( <T>Si<T>, AsisteD:AsisteD.MapaLatitud, AsisteD:AsisteD.MapaLongitud)
VisibleCondicion=(MovTipo(<T>ASIS<T>, Asiste:Asiste.Mov) en (ASIS.R, ASIS.PA, ASIS.A)) y (Asiste:Asiste.Estatus <> EstatusSinAfectar)
...
Expresion=VerImagenBinaria( <T>Imagen<T>, SQL(<T>SELECT Archivo FROM AnexoBase64 WHERE ID =:nIds AND IDR =:nReng<T>, AsisteD:AsisteD.ID, AsisteD:AsisteD.Renglon))
