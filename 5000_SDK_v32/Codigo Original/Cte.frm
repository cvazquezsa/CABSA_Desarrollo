...
AntesExpresiones=Asigna(Info.Cuenta,Cte:Cte.Cliente)<BR>EjecutarSQL(<T>spMESCatalogos :tCatalogo, :tINTCuenta, :nEliminar<T>,<T>CteMES<T>,Info.Cuenta,1)
[Acciones.Navegador]
Nombre=Navegador
Boton=0
...
Antes=S
DespuesGuardar=S
EjecucionCondicion=Asigna(Info.Mensaje,Nulo)<BR>SI(General.ProdInterfazINFOR,Asigna(Info.Mensaje,SQL(<T>SELECT dbo.fnInforValidarCte(:tNombreCorto)<T>,Cte:Cte.NombreCorto)))<BR>vacio(Info.Mensaje)
EjecucionMensaje=Info.Mensaje
AntesExpresiones=Asigna(Info.Cuenta,Cte:Cte.Cliente)<BR>EjecutarSQL(<T>spMESCatalogos :tCatalogo, :tINTCuenta<T>,<T>CteMES<T>,Info.Cuenta)
...
TeclaRapida=Ctrl+H
Antes=S
Expresion=Asigna(Info.Tabla, <T>Cte<T>)<BR>Asigna(Info.Llave, Carpeta.LlavePrimaria(<T>Ficha<T>, <T>|<T>))<BR>Si<BR> SQL(<T>SELECT RegHist FROM Version<T>) y (SQL(<T>SELECT Estatus FROM CfgRegHist WHERE SysTabla=:tTabla<T>, Info.Tabla)=EstatusActivo)<BR>Entonces<BR> Forma(<T>CteHist<T>)<BR>Sino<BR> Informacion(<T>Es necesario activar la opción Registro Cambios Históricos (Automático) en la configuración de Versión<T>)<BR>Fin
EjecucionCondicion=ConDatos(Cte:Cte.Cliente)
AntesExpresiones=Asigna(Info.Cliente, Cte:Cte.Cliente)<BR>Asigna(Info.Nombre, Cte:Cte.Nombre)
...
Cte.MapaPrecision=Cte.TieneMovimientos
Cte.TieneMovimientos=(Fin)
...
Cte.Estado=Cte.Pais
Cte.Pais=Cte.MapaUbicacion
Cte.MapaUbicacion=Cte.Telefonos
...
[Ficha.Cte.MapaUbicacion]
Carpeta=Ficha
Clave=Cte.MapaUbicacion
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
