...
ExpresionesAlMostrar=Asigna(Info.ClienteD, SQL(<T>SELECT MIN(Cliente) FROM Cte<T>))<BR>Asigna(Info.ClienteA, SQL(<T>SELECT MAX(Cliente) FROM Cte<T>))<BR>Asigna(Info.Moneda, <T>(Todas)<T>)<BR>Asigna(Rep.Titulo, <T>Antigüedad de Saldos - Cuentas por Cobrar<T>)<BR>Asigna(Info.FechaD, PrimerDiaAño(Hoy))<BR>Asigna(Info.FechaA, Hoy)
[(Variables)]
Estilo=Ficha
Clave=(Variables)
PermiteEditar=S
