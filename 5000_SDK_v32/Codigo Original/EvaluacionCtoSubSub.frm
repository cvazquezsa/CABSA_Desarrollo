[Forma]
Clave=EvaluacionCtoSubSub
Nombre=<T>Evaluación - <T>+Info.SubSubEvaluacion+ <T> (<T>+FechaEnTexto(Info.Fecha, <T>dd/mmm/aaaa<T>)+<T>)<T>
Icono=0
Modulos=(Todos)
ListaCarpetas=(Lista)
...
VentanaExclusiva=S
AutoGuardar=S
ExpresionesAlMostrar=EjecutarSQL(<T>spEvaluacionCto :tEval, :fFecha, :tCto<T>, Info.SubSubEvaluacion, Info.Fecha, Info.Clave)
...
FiltroGeneral=EvaluacionCto.Contacto=<T>{Info.Clave}<T> AND <BR>EvaluacionCto.Fecha=<T>{FechaFormatoServidor(Info.Fecha)}<T> AND<BR>EvaluacionCto.Evaluacion=<T>{Info.SubSubEvaluacion}<T>
...
Antes=S
DespuesGuardar=S
AntesExpresiones=EjecutarSQL(<T>spEvaluacionReferencia :tReferencia, :tContacto, :fFecha<T>, Info.SubSubEvaluacion, Info.Clave, Info.Fecha)
...
Visible=S
NombreEnBoton=S
Expresion=Asigna(Info.SubSubSubEvaluacion, EvaluacionCto:EvaluacionFormato.Referencia)<BR>Forma(<T>EvaluacionCtoSubSubSub<T>)<BR>Forma.ActualizarVista
