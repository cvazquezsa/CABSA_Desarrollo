[Forma]
Clave=mis_RepInvAnalisisMovDetalle
Nombre=General de Movimientos Detalle  ( Inventarios )
Icono=0
Modulos=(Todos)
...
ExpresionesAlMostrar=Asigna(Info.ArticuloD, SQL(<T>SELECT Min(Articulo) FROM Art<T>))<BR>Asigna(Info.ArticuloA, SQL(<T>SELECT Max(Articulo) FROM Art<T>))<BR>Asigna(Info.MovClaveAfecta, <T><T>)<BR>Asigna(Info.Modulo, <T>INV<T>)<BR>Asigna(Info.Almacen, <T>(Todos)<T>)<BR>Asigna(Info.ArtCat, <T>(Todos)<T>)<BR>Asigna(Info.ArtGrupo, <T>(Todos)<T>)<BR>Asigna(Info.ArtFam, <T>(Todos)<T>)<BR>Asigna(Info.ArtLinea, <T>(Todos)<T>)<BR>Asigna(Info.Fabricante, <T>(Todos)<T>)<BR>Asigna(Info.Estatus, <T>Concluido<T>)<BR>Asigna(Info.Sucursal, Nulo)<BR>Asigna(Info.UEN, Nulo)<BR>Asigna(Info.Desglosar, <T>No<T>)
[(Variables)]
Estilo=Ficha
Clave=(Variables)
...
ListaAccionesMultiples=Variables Asignar<BR>mis_InvAnalisisMovDetalle
Activo=S
Visible=S
...
[Acciones.Imprimir.mis_InvAnalisisMovDetalle]
Nombre=mis_InvAnalisisMovDetalle
Boton=0
TipoAccion=Reportes Impresora
ClaveAccion=mis_InvAnalisisMovDetalle

[Acciones.Preliminar.mis_InvAnalisisMovDetalle]
Nombre=mis_InvAnalisisMovDetalle
Boton=0
...
Info.MovClaveAfectaRI=Info.Estatus
