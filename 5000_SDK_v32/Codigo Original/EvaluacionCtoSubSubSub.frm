[Forma]
Clave=EvaluacionCtoSubSubSub
Nombre=<T>Evaluación - <T>+Info.SubSubSubEvaluacion+ <T> (<T>+FechaEnTexto(Info.Fecha, <T>dd/mmm/aaaa<T>)+<T>)<T>
Icono=0
Modulos=(Todos)
ListaCarpetas=(Lista)
...
VentanaExclusiva=S
AutoGuardar=S
ExpresionesAlMostrar=EjecutarSQL(<T>spEvaluacionCto :tEval, :fFecha, :tCto<T>, Info.SubSubSubEvaluacion, Info.Fecha, Info.Clave)
...
FiltroGeneral=EvaluacionCto.Contacto=<T>{Info.Clave}<T> AND <BR>EvaluacionCto.Fecha=<T>{FechaFormatoServidor(Info.Fecha)}<T> AND<BR>EvaluacionCto.Evaluacion=<T>{Info.SubSubSubEvaluacion}<T>
...
Antes=S
DespuesGuardar=S
AntesExpresiones=EjecutarSQL(<T>spEvaluacionReferencia :tReferencia, :tContacto, :fFecha<T>, Info.SubSubSubEvaluacion, Info.Clave, Info.Fecha)
