[Forma]
Clave=ExplorarIEPSFiscalDinero
Nombre=Explorando - IEPS Fiscal (Bancos)
Icono=47
Modulos=(Todos)
ListaCarpetas=Lista
...
FiltroGeneral=Auxiliar.Empresa=<T>{Empresa}<T> AND Dinero.IEPSFiscal<>0 <BR>AND CtaDinero.Tipo=<T>Banco<T><BR>{Si(Info.Base=<T>S<T>, <T> AND Auxiliar.Conciliado = 1<T>, <T><T>)}<BR>{Si(Info.Base=<T>N<T>, <T> AND Auxiliar.Conciliado = 0<T>, <T><T>)}
