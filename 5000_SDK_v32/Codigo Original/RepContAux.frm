...
ExpresionesAlMostrar=Asigna(Info.CuentaD, SQL(<T>SELECT MIN(Cuenta) FROM Cta WHERE EsAcumulativa = 0<T>))<BR>Asigna(Info.CuentaA, SQL(<T>SELECT MAX(Cuenta) FROM Cta WHERE EsAcumulativa = 0<T>))<BR>Asigna(Info.FechaD, PrimerDiaMes)<BR>Asigna(Info.FechaA, UltimoDiaMes )<BR>Asigna(Info.Moneda, Config.ContMoneda)<BR>Asigna(Rep.Aux, <T>Concentrado<T>)<BR>Asigna(Rep.MovEspecifico, <T>(Todos)<T>)<BR>Asigna(Info.CtaCat, <T>(Todos)<T>)<BR>Asigna(Info.CtaFam, <T>(Todos)<T>)<BR>Asigna(Info.CtaGrupo, <T>(Todos)<T>)<BR>Asigna(Rep.Titulo, e(<T>Auxiliares - Contabilidad<T>))<BR>Asigna(Info.Sucursal,Nulo)
[(Variables)]
Estilo=Ficha
Clave=(Variables)
