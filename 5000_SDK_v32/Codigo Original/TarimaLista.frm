...
ExpresionesAlMostrar=Si<BR>    Info.Clave = <T>INV.T<T><BR>Entonces<BR>    Si<BR>        Info.SubMovTipo = <T>0<T><BR>    Entonces<BR>        Asigna(Info.Tipo,Comillas(<T>Surtido<T>))<BR>    SiNo<BR>        Asigna(Info.Tipo,<T>AlmPos.Tipo<T>)<BR>    Fin<BR>SiNo<BR>    Asigna(Info.Tipo,<T>AlmPos.Tipo<T>)<BR>Fin<BR>ActualizarForma<BR>ActualizarVista
[Lista]
Estilo=Iconos
Clave=Lista
...
FiltroGeneral=Tarima.Almacen=<T>{Info.Almacen}<T> AND ArtDisponibleTarima.Articulo = (SELECT ISNULL(NULLIF(CONVERT(varchar(20),<T>{Info.Articulo}<T>),<T><T>),ArtDisponibleTarima.Articulo))<BR>AND ArtDisponibleTarima.Disponible>0 AND AlmPos.Tipo = {Info.Tipo}
[Lista.Columnas]
0=125
1=129
...
(Inicio)=Tarima.FechaCaducidad
Tarima.FechaCaducidad=Tarima.Almacen
Tarima.Almacen=Tarima.Posicion
Tarima.Posicion=ArtDisponibleTarima.Articulo
ArtDisponibleTarima.Articulo=Tarima.SubCuenta
