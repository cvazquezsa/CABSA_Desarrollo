...
ExpresionesAlMostrar=Asigna(Info.ProveedorD,SQL(<T>SELECT Min(Proveedor) From Prov<T> ))<BR>Asigna(Info.ProveedorA,SQL(<T>SELECT Max(Proveedor) From Prov<T> ))<BR>Asigna(Info.ProvCat,<T>(Todos)<T>)<BR>Asigna(Info.ProvFam,<T>(Todos)<T>)<BR>Asigna(Info.Desglosar,<T>Si<T>)<BR>Asigna(Info.Proyecto,<T>(Todos)<T>)
