[Forma]
Clave=mis_RepVentaAnalisisMovDiario
...
ListaAccionesMultiples=Variables Asignar<BR>mis_VentaAnalisisMovDiario
Activo=S
Visible=S
...
[Acciones.Imprimir.mis_VentaAnalisisMovDiario]
Nombre=mis_VentaAnalisisMovDiario
Boton=0
TipoAccion=Reportes Impresora
ClaveAccion=mis_VentaAnalisisMovDiario

[Acciones.Preliminar.mis_VentaAnalisisMovDiario]
Nombre=mis_VentaAnalisisMovDiario
Boton=0
...
Activo=S
Visible=S
Expresion=Browser(General.RSReportASPX+<T>/DiarioMovimientos/Diariomovimientos&Empresa=<T>+Info.Empresa+<T>&ClienteD=<T>+Info.ClienteD+<T>&ClienteA=<T>+Info.ClienteA+<T>&CatCte=<T>+Info.CteCat+<T>&FamCte=<T>+Info.CteFam+<T>&GpoCte=<T>+Info.CteGrupo+<T>&Sucursal=<T>+Info.Sucursal+<T>&UEN=<T>+Info.UEN+<T>&Mov=<T>+Info.MovClaveAfecta+<T>&Estatus=<T>+Info.Estatus+<T>&Almacen=<T>+Info.Almacen+<T>&FechaD=<T>+FechaFormatoServidor(Info.FechaD)+<T>&FechaA=<T>+FechaFormatoServidor(Info.FechaA)+<T>&rs:ClearSession=true&rs:Format=HTML4.0&rs:Command=Render&rc:Area=ToolBar&rs:PrevPage=Enabled&rs:NextPage=Enabled&rs:FirstPage=Enabled&rc:LinkTarget=_top&rc:JavaScript=True&rc:Toolbar=True<T>, <T>Ventas - Diario de movimientos<T>)
...
Info.MovClaveAfectaRV=Info.Estatus
