[Forma]
Clave=mis_RepAgentAnalisisMov
Nombre=General de Movimientos (Agente)
Icono=0
Modulos=(Todos)
...
ExpresionesAlMostrar=Asigna(Info.AgenteD, SQL(<T>SELECT Min(Agente) FROM Agente<T>))<BR>Asigna(Info.AgenteA, SQL(<T>SELECT Max(Agente) FROM Agente<T>))<BR>Asigna(Info.Moneda, Config.ContMoneda)<BR>Asigna(Info.Estatus, <T>Concluido<T>)<BR>Asigna(Info.Sucursal, Nulo)<BR>Asigna(Info.Modulo, <T>AGENT<T>)<BR>Asigna(Info.MovClaveAfecta, <T><T>)<BR>Asigna(Info.UEN, Nulo)
[(Variables)]
Estilo=Ficha
Clave=(Variables)
...
ListaAccionesMultiples=Variables Asignar<BR>mis_AgentAnalisisMov
Activo=S
Visible=S
EnBarraHerramientas=S
EspacioPrevio=S
...
[Acciones.Imprimir.mis_AgentAnalisisMov]
Nombre=mis_AgentAnalisisMov
Boton=0
TipoAccion=Reportes Impresora
ClaveAccion=mis_AgentAnalisisMov

[Acciones.Preliminar.mis_AgentAnalisisMov]
Nombre=mis_AgentAnalisisMov
Boton=0
