[Forma]
Clave=EvaluacionCto
Nombre=e(<T>Evaluación<T>)+<T> - <T>+Info.Evaluacion+ <T> (<T>+FechaEnTexto(Info.Fecha, <T>dd/mmm/aaaa<T>)+<T>)<T>
Icono=0
Modulos=(Todos)
ListaCarpetas=(Lista)
...
Antes=S
DespuesGuardar=S
AntesExpresiones=EjecutarSQL(<T>spEvaluacionReferencia :tReferencia, :tContacto, :fFecha<T>, Info.Evaluacion, Info.Clave, Info.Fecha)
...
Visible=S
GuardarAntes=S
Expresion=Asigna(Info.SubEvaluacion, EvaluacionCto:EvaluacionFormato.Referencia)<BR>Forma(<T>EvaluacionCtoSub<T>)<BR>Forma.ActualizarVista
