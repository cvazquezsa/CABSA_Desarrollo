[Forma]
Clave=RepCxpPronostico
Nombre=Pronóstico de Pagos
Icono=0
Modulos=(Todos)
...
ExpresionesAlMostrar=Asigna(Info.ProveedorD, SQL(<T>SELECT MIN(Proveedor) FROM Prov<T>))<BR>Asigna(Info.ProveedorA, SQL(<T>SELECT MAX(Proveedor) FROM Prov<T>))<BR>Asigna(Info.Desglosar, <T>Si<T>)<BR>Asigna(Info.Moneda, <T>(Todas)<T>)<BR>Asigna(Info.Ano, Año(Hoy))<BR>Asigna(Rep.Titulo, <T>Pronóstico de Pagos<T>)
[(Variables)]
Estilo=Ficha
Clave=(Variables)
PermiteEditar=S
