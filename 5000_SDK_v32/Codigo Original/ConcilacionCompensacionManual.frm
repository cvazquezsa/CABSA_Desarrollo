[Forma]
Clave=ConcilacionCompensacionManual
...
FiltroGeneral=Dinero.Empresa=<T>{Empresa}<T><BR>AND Dinero.Estatus IN (<T>PENDIENTE<T>, <T>CONCLUIDO<T>)<BR>AND Auxiliar.Cuenta=<T>{Info.CtaDinero}<T> <BR>AND Auxiliar.Conciliado = 0<BR>AND Auxiliar.ID NOT IN (SELECT Manual FROM ConciliacionD WHERE ID={Info.ID})
