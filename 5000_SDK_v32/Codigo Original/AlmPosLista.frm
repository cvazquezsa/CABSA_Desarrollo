...
FiltroGeneral=AlmPos.Almacen=<T>{Info.Almacen}<T><BR>{Si(ConDatos(Info.Mensaje), <T>AND AlmPos.Tipo=<T>+Comillas(Info.Mensaje),<T><T>)}<BR>{Si(ConDatos(Info.Tipo), <T>AND AlmPos.Tipo=<T>+Comillas(Info.Tipo),<T><T>)}
