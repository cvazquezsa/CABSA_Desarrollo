...
Comentarios=Lista(Vacio(Info.Lista, <T>(Todos los Artículos)<T>), Vacio(Info.Proveedor, <T>(Todos los Proveedores)<T>))
...
FiltroGeneral=Art.Tipo<><T>Estructura<T><BR>{Si(ConDatos(Info.Lista), <T> AND Art.Articulo IN (SELECT DISTINCT Articulo FROM ListaPreciosD WHERE Lista = <T>+Comillas(Info.Lista)+<T>)<T>, <T><T>)}<BR>{Si(ConDatos(Info.Proveedor), <T> AND Art.Proveedor=<T>+Comillas(Info.Proveedor), <T><T>)}
...
Visible=S
Antes=S
AntesExpresiones=RegistrarSeleccion(<T>Lista<T>)<BR>EjecutarSQL(<T>spPCArtListaAceptar :nEstacion, :nID<T>, EstacionTrabajo, Info.ID)
...
Boton=50
NombreEnBoton=S
NombreDesplegar=&Ver Todos Artículos
EnBarraHerramientas=S
EspacioPrevio=S
TipoAccion=Controles Captura
ClaveAccion=Actualizar Forma
Visible=S
ActivoCondicion=ConDatos(Info.Lista)
Antes=S
AntesExpresiones=Asigna(Info.Lista, Nulo)
