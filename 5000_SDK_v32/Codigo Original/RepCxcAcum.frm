[Forma]
Clave=RepCxcAcum
Nombre=Acumulados - Cuentas por Cobrar
Icono=0
Modulos=(Todos)
...
ExpresionesAlMostrar=Asigna(Info.ClienteD, SQL(<T>SELECT MIN(Cliente) FROM Cte<T>))<BR>Asigna(Info.ClienteA, SQL(<T>SELECT MAX(Cliente) FROM Cte<T>))<BR>Asigna(Info.Ejercicio, EjercicioTrabajo)<BR>Asigna(Info.ConAcumulados, <T>Si<T>)<BR>Asigna(Info.Desglosar, <T>Si<T>)<BR>Asigna(Info.Moneda, <T>(Todas)<T>)<BR>Asigna(Info.CteCat, <T>(Todos)<T>)<BR>Asigna(Info.CteFam, <T>(Todos)<T>)<BR>Asigna(Info.CteGrupo, <T>(Todos)<T>)<BR>Asigna(Rep.Titulo, <T>Acumulados - Cuentas por Cobrar<T>)
[(Variables)]
Estilo=Ficha
Clave=(Variables)
PermiteEditar=S
