[Forma]
Clave=ExplorarIVAFiscal
Nombre=Explorando - IVA Fiscal (Cxc/Cxp)
Icono=47
Modulos=(Todos)
ListaCarpetas=Lista
...
FiltroFechasCampo=IVAFiscal.AplicaFecha
...
FiltroGeneral=IVAFiscal.Empresa=<T>{Empresa}<T> <BR>AND IVAFiscal.IVAFiscalImporte<>0 <BR>AND IVAFiscal.FechaEmision>=<T>{FechaFormatoServidor(Config.FiscalInicioIVA)}<T>
