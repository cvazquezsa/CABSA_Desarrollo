SELECT
      ...
      +'&tt='+ISNULL(CONVERT(VARCHAR(25),dbo.fnCFDFlexFormatearImporte(Venta.Importe+ISNULL(Venta.Impuestos,0)-ISNULL(Venta.Retencion,0),10,6)),'0.00')
	  ...
	  CFO.noAprobacion AS noAprobacion,
	  CFO.fechaAprobacion AS FechaAprobacion,  
	  Empresa.FiscalRegimen AS RegimenFiscal,  --Regimen Fiscal (clave)
	  FIR.Descripcion AS DesRegimen,        --Descripcion del regimen fiscal 
	  COND.CFD_metodoDePago AS MetodoDePago, ---Metodo de Pago (PUE o PPD)
	  CFD.Fecha AS FechaExpedicion,
	  sat.ClaveUsoCFDI+' '+sat.Descripcion AS ClaveUsoCFDI,
	  'Calle:'+Empresa.Direccion+' '+'No.'+Empresa.DireccionNumero+' ' +'Colonia: '+Empresa.Colonia+' '+'C.P '+Empresa.CodigoPostal+' '+'Estado: '+Empresa.Estado+' '+'País: '+Empresa.Pais AS 'DIreccionEmisor', 
	  'Calle:'+Cte.Direccion+' '+'No.'+Cte.DireccionNumero+' '+'Colonia:'+Cte.Colonia+' '+'C.P. '+Cte.CodigoPostal+' '+'Delegación: '+Cte.Delegacion+' '+'Estado:'+Cte.Estado+' País:'+Cte.Pais AS 'DireccionReceptor'
  FROM Venta
  ...
  LEFT JOIN CFDFolio CFO  ON ISNULL(Venta.Empresa,'')=CFO.Empresa AND Venta.Mov=CFO.Mov--ON CFO.Modulo=CFD.Modulo AND CFO.Mov=Venta.Mov
  ...
  LEFT JOIN FiscalRegimen FIR ON FIR.FiscalRegimen=Empresa.FiscalRegimen
  LEFT JOIN Condicion COND ON COND.Condicion=Venta.Condicion
  LEFT JOIN CteCFD ccfd ON ccfd.Cliente=Venta.Cliente
  LEFT JOIN SATCatUsoCFDI sat ON sat.ClaveUsoCFDI=ccfd.ClaveUsoCFDI
  WHERE Venta.ID = :ID
